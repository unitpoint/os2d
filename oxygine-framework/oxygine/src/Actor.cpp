...
namespace oxygine
{
	...
	void Actor::insertChildAfter(spActor actor, spActor insertAfter)
	{
		...
		ObjectScript::registerActorChild(this, actor.get());
	}
	...
	void Actor::removeChild(spActor actor)
	{
		...
		{	
			...
			{
				ObjectScript::unregisterActorChild(this, actor.get());
	...
	void Actor::internalUpdate(const UpdateState &us)
	{
		...
		while(tween)
		{
			...
			if (tween->isDone() && tween->getParentList()){
				ObjectScript::unregisterTween(this, tween.get());
	...
	spTween Actor::_addTween(spTween tween, bool rel)
	{
		...
		ObjectScript::registerTween(this, tween.get());
	...
	void Actor::removeTween(spTween v)
	{
		...
		{
			v->setClient(0);
			ObjectScript::unregisterTween(this, v.get());
			_tweens.remove(v);
		}
	}
